"""
## * Worksession export to Keyshot. The same functionality as '_UpdateKeyshot6'
...
def export_to_keyshot(bool_export, coll_objs):
    """
    Exports the created objects to Keyshot 6.
    Parameters:
        bool_export = (Boolean) Activate the method.
        coll_objs = (List) with created objects, used for clean_up method.
    """
    if not bool_export:
        print("Turn on bool_export")
        return
    elif coll_objs is not None:
        rc = rs.Command('_UpdateKeyshot6')
        if not rc: return
        else: 
